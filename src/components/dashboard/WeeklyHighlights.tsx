  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("ai_insights").select("*").order("created_at", {
        ascending: false
      }).limit(1).single();
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from("ai_logs").select("id").eq("type", "insight").contains("payload", {
  ...
  return <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button onClick={handleGenerate} disabled={isGenerating || isLoading} size="sm" variant="outline">
